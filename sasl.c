...
static int sasl_callback(LDAP *ldap_cnx, unsigned flags, void *_defaults, void *in) {
...
// TODO where do we free defaults
napi_value
sasl_bind (napi_env env, napi_callback_info info) {
  ...
  size_t size = 0;
    ...
    if (vt != napi_string) {
        ...
        status = napi_get_value_string_utf8 (env, argv[0], defaults->mechanism, size, &size);
        assert (status == napi_ok);
    }
  ...
  puts("2.2");
    if (vt != napi_string) {
        ...
        status = napi_get_value_string_utf8 (env, argv[1], defaults->user, size, &size);
        assert (status == napi_ok);
    }
  ...
  puts("3");
    ...
    if (vt != napi_string) {
        ...
        status = napi_get_value_string_utf8 (env, argv[2], defaults-> password, size, &size);
        assert (status == napi_ok);
    }
    ...
    if (vt != napi_string) {
        ...
        status = napi_get_value_string_utf8 (env, argv[3], defaults->realm, size, &size);
        assert (status == napi_ok);
  ...
  puts("4");
    ...
    if (vt != napi_string) {
        ...
        status = napi_get_value_string_utf8 (env, argv[4], defaults->proxy_user, size, &size);
        assert (status == napi_ok);
    ...
    if (vt != napi_string) {
        ...
        status = napi_get_value_string_utf8 (env, argv[5], defaults->sec_props, size, &size);
        assert (status == napi_ok);

    }

    if (defaults->sec_props) {
        int res = ldap_set_option(ldap_cnx->ld, LDAP_OPT_X_SASL_SECPROPS, defaults->sec_props);
        if(res != LDAP_SUCCESS) {
  ...
  int res = ldap_sasl_interactive_bind(ldap_cnx->ld, NULL, defaults->mechanism,
